export default class GameFunctions {
    ...
    evaluateDiceResult(gameState, playerId) {
        ...
        for (let i = 1; i < numDice; i++) {
            const numberText = this.numberToText(i, false);

            if (newCombos[numberText][player] === -1) {
                const score = this.diceScore(counts, i, true);
                if (score > 0) {
    ...
    diceScore(counts, val, single) {
        ...
        if (!single) {
            ...
            for (const property in counts) {
                if (property === val.toString()) {
                    let score = counts[property] * val
                    numbers.push(score);
                }
            }
        }
